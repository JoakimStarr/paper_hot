...
**Extended deps load on demand (by design).** Crawler libs and heavy compute libs (jieba/sklearn/numpy) are imported inside functions (or in modules that are only lazily imported), so a crawler-less/minimal server boots fast and never blocks on them. The frontend equivalents: mermaid/docx are dynamic-imported; D3 is subpath-imported only by the network page. Preserve this pattern when adding dependencies.
...
- **`backend/.env` must only contain keys declared in `Settings`** (pydantic-settings forbids extra fields). All supported keys are documented in `backend/.env.example`.
- **User-configured settings live in the database, not `.env`.** The system page persists overrides to the `system_settings` table (via `app/settings_store.py`). Priority: DB > env vars > `.env` baseline. Overrides are applied at import time of `app.config` (before the FastAPI app is built), so CORS/ports/app_name reflect them immediately. Port keys are additionally mirrored to `.env` because `start.sh` reads ports before Python starts. Do not reintroduce direct `.env` writes.
...
- **Blocking IO inside async code is forbidden.** All sync OpenAI client calls, embedding calls, and DrissionPage browser flows must go through `asyncio.to_thread` / `*_async` wrappers (see `ai_service.embed_texts_async`, `ai_processor.compute_embedding_async`, agent tool loop). Direct sync calls in coroutines froze the whole API in the past.
- **Scoring has ONE formula** (0.35/0.35/0.30, 180-day half-life) owned by `crud.PaperCRUD`. `scoring.ScoringSystem` is a thin delegating wrapper; `compute_should_read_score` (second formula) was removed on 2026-08-27.
