#!/usr/bin/env python3
"""
知网爬虫 - 参考文献抓取副本（cnki_paper_captcha.py 的独立副本，主脚本保持不变）

本文件完整复制 cnki_paper_captcha.py，并新增 ReferenceCrawler：
从论文详情页抓取参考文献列表 —— 进入详情页先点击「参考文献」页签
...
结果按 paper_url 覆盖式写入 paper_references 表（表由本脚本自建，与 backend 模型解耦）。
...
使用方法:
    python cnki_paper_references.py --show-browser
    python cnki_paper_references.py --threads 3
    # 参考文献抓取（本副本新增）
    python cnki_paper_references.py --ref-paper-url "https://kns.cnki.net/kcms2/article/abstract?..."
    python cnki_paper_references.py --ref-urls-file .cache/urls.txt --ref-max-items 200
    python cnki_paper_references.py --ref-title "论文标题"
"""
...
class ReferenceCrawler(KeywordSearchCrawler):
    ...
    用法:
        python cnki_paper_references.py --ref-paper-url "https://kns.cnki.net/..."
        python cnki_paper_references.py --ref-urls-file .cache/urls.txt
        python cnki_paper_references.py --ref-title "论文标题" --ref-max-items 100
    """
...
async def main():
    parser = argparse.ArgumentParser(description='知网爬虫副本 - 验证码自动解决版本（按期刊 / 按关键词检索 / 参考文献抓取）')
